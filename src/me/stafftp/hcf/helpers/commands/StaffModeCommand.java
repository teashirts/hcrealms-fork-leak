...
import net.aodus.hcf.staffmode.StaffItems;
...
public class StaffModeCommand implements Listener, CommandExecutor {
	...
	public static boolean enterMod(final Player p) {
		...
		p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&6Mod Mode: &aEnabled" ));
//sender.sendMessage(ChatColor.AQUA + ChatColor.BOLD.toString() + ChatColor.GOLD + "Sub Requirement" + " » " + ChatColor.WHITE + Base.getPlugin(Base.class).getConfig().getString("youtube_subs"));
	...
	public static boolean leaveMod(final Player p) {
		...
		p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&6Mod Mode: &cDisabled"));
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (cmd.getName().equalsIgnoreCase("staffmode")) {
			...
			if (!sender.hasPermission("command.mod.others")) {
				sender.sendMessage(ChatColor.RED + "No permission to give this player staff mode.");
				return true;
			}
			...
			if (modMode.contains(t)) {
				leaveMod(t);
				sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&7You have &cdisabled &7" + t.getName() + "'s &6&lStaff Mode"));
				return true;
			}
			enterMod(t);
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&7You have &aenabled &7" + t.getName() + "'s &6&lStaff Mode"));
			return true;
		}
		return false;
	}
