package me.stafftp.hcf.special.TripleKey;
...
public class TripleKeyCommand implements CommandExecutor, TabCompleter {

	private final Base plugin;

	public TripleKeyCommand(Base plugin) {
		this.plugin = plugin;
	}
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length == 0 || args.length > 2) {
            sender.sendMessage(
                    "§9§m--------------------------------");
            sender.sendMessage(
            		"§c/triplekey start <duration> - Starts the KeySale timer.");
            sender.sendMessage(
            		"§c/triplekey end - Ends the current sale timer.");
            sender.sendMessage(
                    "§9§m--------------------------------");
			return true;
		}
		...
		if (args.length == 2) {
			if (args[0].equalsIgnoreCase("start")) {
				...
				TripleKeyTimer triplekeytimer = plugin.getTimerManager().getTripleKeyTimer();

				if (!triplekeytimer.setRemaining(duration, true)) {
				...
				Bukkit.broadcastMessage(ChatColor.GRAY + "The " + ChatColor.GREEN.toString() + ChatColor.BOLD + "TripleKey Sale " + ChatColor.GRAY + "has started at " + ChatColor.DARK_GREEN + ConfigurationService.DONATE_URL
